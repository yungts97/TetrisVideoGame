...
{
	public class HorizontalBombBoard:Board
	{
		private Label txtTitle;
		private Label txtQty;
		private PictureBox picBomb;

		public HorizontalBombBoard(Form myform, int blocksize, int col, int row) : base(blocksize, col, row)
		{
			initialize(myform);
		}
		public override void initialize(Form form)
		{
			...
			/*for (int i = 0; i < _rows; ++i)
			{
				for (int j = 0; j < _columns; ++j)
				{
					...
					grids[i, j].Top = 500 + i * _blockSize;
			...
			picBomb.Image = Image.FromFile("horizontalBomb.png");
			picBomb.Top = 520;
			picBomb.Left = 120;
