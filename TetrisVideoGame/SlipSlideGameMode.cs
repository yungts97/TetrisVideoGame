...
{
    class SlipSlideGameMode : Form
    {
        PlayFieldBoard _playboard;
        ...
        public void init()
        {
            ...
            GameTimer.Interval = time_interval - (_player.Level * 570);
        ...
        private void GameTimer_Tick(object sender, EventArgs e)
        {
            if (!_collisionDetector.CheckCollision(_tetromino, _playboard, 0, 1))
            {
                ...
                if (directionLeft)
                {
                    if (_tetromino.PositionX + _tetromino.Width <= 9)
                        _tetromino.PositionX += 1;
                    else
                    {
